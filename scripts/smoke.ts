...
const checks: Check[] = [
  ...
  { name: "model detail provider filter link", run: bodyIncludes("/models/sample-acme-ai-sample-alpha-chat", "/models?provider=") },
  { name: "alerts unsubscribe 200", run: status("/alerts/unsubscribe", 200) },
  { name: "alerts verify 200", run: status("/alerts/verify", 200) },
